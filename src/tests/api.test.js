...
describe('Authors', () => {
	...
	// ----- NUEVOS TESTS PUT -----
	it('PUT /authors/:id válido -> 200', async () => {
		const mockUpdatedAuthor = { id: 1, name: 'Ana Actualizada', email: 'ana@example.com', bio: 'Nueva bio' };
		authorsService.getAuthorById.mockResolvedValue({ id: 1 });
		authorsService.findAuthorByEmail.mockResolvedValue(null);
		authorsService.updateAuthor.mockResolvedValue(mockUpdatedAuthor);
	...
	it('PUT /authors/:id inexistente -> 404', async () => {
		authorsService.getAuthorById.mockResolvedValue(null);

		const res = await request(app)
			.put('/authors/999')
			.send({ name: 'Cualquiera', email: 'cualquiera@example.com' });

		expect(res.status).toBe(404);
		expect(res.body).toHaveProperty('error', 'Author no encontrado');
	});
});
...
describe('Posts', () => {
	...
	// ----- NUEVOS TESTS PUT -----
	it('PUT /posts/:id válido -> 200', async () => {
		const mockUpdatedPost = { id: 1, title: 'Título actualizado', content: 'Contenido nuevo', author_id: 1, published: true };
		...
		const res = await request(app)
			.put('/posts/1')
			.send({ title: 'Título actualizado', content: 'Contenido nuevo', author_id: 1, published: true });

		expect(res.status).toBe(200);
		expect(res.body).toEqual(mockUpdatedPost);
	});

	it('PUT /posts/:id inexistente -> 404', async () => {
		postsService.getPostById.mockResolvedValue(null);

		const res = await request(app)
			.put('/posts/999')
			.send({ title: 'Cualquiera', content: 'Cualquiera', author_id: 1 });
